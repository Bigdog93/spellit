...
const OpenViduVideo = () => {
  ...
  const [mainStreamManager, setMainStreamManager] = useState<
    Publisher | undefined
  >(undefined); // Main video of the page. Will be the 'publisher' or one of the 'subscribers'
  ...
  const createSession = async (sessionId: string) => {
    const response = await axios.post(
      APPLICATION_SERVER_URL + "ov/sessions",
  ...
  const createToken = async (sessionId: number) => {
    const response = await axios.post(
      APPLICATION_SERVER_URL + "ov/sessions/" + sessionId + "/connections",
      {},
      {
        headers: { "Content-Type": "application/json" },
      }
  ...
  return (
    <>
      {mainStreamManager && (
        <OvVideo streamManager={mainStreamManager}></OvVideo>
      )}
